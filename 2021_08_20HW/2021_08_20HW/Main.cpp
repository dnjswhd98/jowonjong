...
int main(void)
{
	...
	//StudentScoreList.insert(make_pair("S 학교", Temp));
	//Output(&StudentScoreList, &Temp);
	Temp.clear();
	...
	//StudentScoreList.insert(make_pair("G 학교", Temp));
	//Output(&StudentScoreList, &Temp);
	Temp.clear();
	...
	//StudentScoreList.insert(make_pair("A 학교", Temp));
	//Output(&StudentScoreList, &Temp);
	//Temp.clear();
...
void Output(map<string, list<StudentScore*>>* _pStudentScoreList)
{
	...
	{	
		...
		for (list<StudentScore*>::iterator liter = Temp.begin();
			liter != Temp.end(); ++liter)
		{
			cout << "이름 : " << (*liter)->Name << endl;
			cout << "국어 점수 : " << (*liter)->Kor << endl;
			cout << "영어 점수 : " << (*liter)->Eng << endl;
			cout << "수학 점수 : " << (*liter)->Math << endl << endl;
		}
	}
}